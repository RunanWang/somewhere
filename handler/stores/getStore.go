...
func GetStoreInfo(c *gin.Context) {
	...
	var (
		getStoreReq  msg.GetStoreInfoReq
		getStoreResp msg.GetStoreInfoResp
		err          error
	)
	...
	list, err := service.GetStoreInfo(c, &getStoreReq)
	if err != nil {
		logger = c.MustGet("logger").(*log.Entry)
		logger = logger.WithFields(log.Fields{
			"error": err.Error(),
...
func GetStoresByPage(c *gin.Context) {

	var (
		getStoreReq  msg.GetStoresByPageReq
		getStoreResp msg.GetStoresResp
		err          error
	)
	...
	list, err := service.GetStoresByPage(c, &getStoreReq)
	if err != nil {
		logger = c.MustGet("logger").(*log.Entry)
		logger = logger.WithFields(log.Fields{
			"error": err.Error(),
