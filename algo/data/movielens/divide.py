...
def write_list(fp, write_list):
    headers = [
        ...
        "Sci-Fi", "Thriller", "War", "Western","gbdt_00","gbdt_01", 
        "gbdt_10","gbdt_11", "gbdt_20","gbdt_21", "gbdt_30","gbdt_31", "gbdt_40","gbdt_41", "gbdt_50","gbdt_51", 
        "timestamp", "label"
    ]
    to_write={}
...
with open('process_train_gbdt.csv', 'w', newline='') as f:
    f_csv = csv.DictWriter(f, headers)
    f_csv.writeheader()

with open('process_test_gbdt.csv', 'w', newline='') as f:
    f_csv = csv.DictWriter(f, headers)
    f_csv.writeheader()

fwrite_train = open('process_train_gbdt.csv', 'a', newline='')
fw_train = csv.DictWriter(fwrite_train, headers)
fwrite_test = open('process_test_gbdt.csv', 'a', newline='')
fw_test = csv.DictWriter(fwrite_test, headers)

with open('gbdt.csv') as f:
    ...
    for row in data:
        if temp_userID != row['UserID']:
            ...
            if temp_count < temp_user_num * 0.8:
                temp_train.append(row)
            else:
